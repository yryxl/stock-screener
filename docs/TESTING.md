...
| 2026-04-22 | BUG-040 | 批量接口遇到 ConnectionReset/网络异常直接返回 None，没重试也没备用源 | data_fetcher.py batch_fetch_with_timeout + get_realtime_quotes | （待提交）| ✅ 已修复（本地 3 组 retry 测试通过）。**根因**：夜间 cron run 24728045222 日志铁证 `批量接口失败: ('Connection aborted.', ConnectionResetError(104, 'Connection reset by peer'))` → 段 1 候选 0 只。原 batch_fetch_with_timeout 在 `except Exception` 分支直接返回 None，不 retry；且没有备用源。GHA IP 经常被东财数据源 reset。**解决方法**：(1) batch_fetch_with_timeout 重写 - 主源 retry 3 次，每次 delay+attempt*2 秒 backoff，区分 _FetchTimeout / ConnectionReset / 其他异常但都触发 retry；(2) 支持 `alt_funcs` 备用源列表，主源全失败才切换；(3) get_realtime_quotes 配置新浪源 `ak.stock_zh_a_spot` 作为东财 `ak.stock_zh_a_spot_em` 的备用。两源都有"代码"和"最新价"列，screener 代码可直接复用。本地 3 组测试：主源全失败/备用源成功/主源直接成功——全部符合预期。 |
