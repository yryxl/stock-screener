...
| 2026-04-21 | BUG-035 | 用户截图 Tab 资产配置："查推荐 ETF"按钮点了没反应 | app.py H2 按钮显示条件 | b1fb9ec | ✅ 已修复 + UX 增强（前端实测过 2 个按钮）。**根因**：(A) 按钮显示条件（偏差 >3pp 绝对值）比推荐生成条件（缺位 ≥5pp）更宽，导致"超目标"（如高股息 +5.8pp）和"临界缺位"（如黄金 -5.0pp）类别显示了按钮但下方根本没对应的 expander；(B) 即使展开正常，rerun 后页面保持滚动位置，展开的 expander 在下方一屏看不到，用户以为"没反应"。**解决方法**：(1) 提前调 `get_recommendations_from_allocation` 拿到真实推荐集合；(2) 按钮仅在 `_ac in _rec_classes` 时显示；(3) 有偏差但没推荐的类别改为 `st.caption` 友好说明（超目标→"应减仓非加仓"，偏差小→"无需补仓"）；(4) 下方 ETF 推荐区复用已算好的 `_recs_cache` 避免重复调用；(5) 点击按钮后注入 JS scrollIntoView 自动滚动到推荐区锚点。 |
| 2026-04-21 | BUG-036 | BUG-034 副作用：批量数据接口被 20s timeout 误杀，段大小从 60 暴涨到 728 致 75min 超时 | data_fetcher.py 4 个批量接口 | 3306699 | ✅ 已修复（被 BUG-037 取代）。**根因**：BUG-034 给 safe_fetch 默认 20s timeout 是为了保护单只调用，但批量接口（stock_yjbb_em 全 ROE / stock_zh_a_spot_em 全行情 / fund_etf_spot_em / stock_info_a_code_name）数据量大，正常需要 30-60s，被 20s 切断后 → fallback 用全市场分流 → 段大小翻 12 倍 → 75min 超时雪崩。日志证据：[超时20s] stock_yjbb_em 出现 2 次 + 段 1 候选从 60 → 728。**解决方法**：4 个批量接口显式传 `timeout=90`，单只调用保持默认 20s。**结果**：段大小恢复到 60，但深度分析仍然超时——因为 china_adjustments.py 里 9 处直接 ak.xxx() 调用没走 safe_fetch，BUG-034 的 SIGALRM 保护不到（log: 09:21:36 之后 69 分钟无进度）。本质问题留给 BUG-037 解决。 |
| 2026-04-21 | BUG-037 | china_adjustments.py 里 9 处直接 ak.xxx() 调用没走 safe_fetch，单只股可能被永远卡死 | data_fetcher.py + screener.py | 7f92165 | ✅ 已修复（run 24718200197 验证：段 1 60 只全跑完 / 0 失败）。**根因**：BUG-034 把 SIGALRM 加到 safe_fetch 内层，但 china_adjustments.py 里 check_cash_loan_double_high / check_drain_business 等函数直接 import akshare as ak; ak.xxx(...)，绕过了 safe_fetch → 没有任何超时保护 → 某只股的 ak.stock_balance_sheet_by_report_em 等调用如果卡住就永远卡（log 显示某次 69 分钟一动不动）。**解决方法**：把 SIGALRM 保护从"单次 akshare 调用"提升到"单只股深度分析"层级——data_fetcher.safe_fetch 移除 BUG-034 的 alarm 逻辑（参数保留但忽略），改在 screener.screen_all_stocks 的 for 循环里给整个 screen_single_stock(code, ...) 调用包 60s SIGALRM。这样无论里面调什么 ak.xxx（包过的、没包的），到 60 秒就被切断。worst case：60s × 60 只 = 60min，留 15min 给批量接口 + commit。 |
| 2026-04-21 | BUG-038 | BUG-037 让段跑完了，但 market_scan_full_p1.json 还是没 push 到 GitHub——git add 是"全有或全无"原子行为 | daily_screen.yml git add 参数列表 | 10ed78f | ✅ 已修复（本地 repro + 真实环境验证）。**根因**：yml 里把 `market_scan_full_p1.json market_scan_full_p2.json ... market_scan_full_p6.json` 6 个显式文件名列出来给 git add。**但每次只跑 1 个段**（比如 full_p1），所以 p2-p6 不存在 → **git add 遇到 pathspec 不匹配会 fatal 退出 128 + 拒绝 stage 任何文件**。本地 repro 验证：`git add existing.json nonexistent.json` → staged 为空。即使有 `2>/dev/null \|\| true` 兜底，也只是让 shell 不退出，但 index 依然是空的。DEBUG 打印显示 "market_scan_full_p1.json 11649 bytes" 明明存在 → git add 后 staged 空 → commit 步骤 "Everything up-to-date"。**解决方法**：yml 的 git add 改成 bash glob + nullglob 扩展——只传入实际存在的文件给 git add。另加 `for f in ...; do [ -f "$f" ] && git add "$f"; done` 保护 tracked 文件。**真实验证**：run 24723677508（full_p1 timeout）最终 market_scan_full_p1.json 成功 push 到 GitHub（3137 bytes, candidates_count:80, is_partial=True）——BUG-032+033+038 联动成功。 |
| 2026-04-21 | BUG-039 | BUG-037 回归：safe_fetch 的 alarm 被移除后批量接口无超时保护，段大小再次暴涨到 728 | data_fetcher.py 批量接口 4 个函数 | 8031a6f | ✅ 已修复（由 BUG-040 闭环：batch_fetch_with_timeout + retry+备用源）。**根因**：BUG-037 为了避免 SIGALRM 嵌套冲突，把 safe_fetch 里的 alarm 全移除了。**解决方法**：新增 `batch_fetch_with_timeout(func, timeout_sec=90)` 工具。**后续发现**：夜间 cron 跑时遇到 `ConnectionResetError(104, 'Connection reset by peer')`——不是超时问题，是 GHA IP 段被东财主动断开。原版代码直接返回 None → 候选 0 只 → 数据为空覆盖。本质问题留给 BUG-040 修复。 |
| 2026-04-22 | BUG-040 | 批量接口遇到 ConnectionReset/网络异常直接返回 None，没重试也没备用源 | data_fetcher.py batch_fetch_with_timeout + get_realtime_quotes | abc57a9 | ✅ 已修复（真实 run 24759960954 验证通过）。**根因**：夜间 cron run 24728045222 日志铁证 `批量接口失败: ('Connection aborted.', ConnectionResetError(104, 'Connection reset by peer'))` → 段 1 候选 0 只。GHA IP 经常被东财数据源 reset。**解决方法**：(1) batch_fetch_with_timeout 重写 - 主源 retry 3 次 + backoff；(2) 支持 `alt_funcs` 备用源；(3) get_realtime_quotes 配置新浪源 `ak.stock_zh_a_spot` 作为东财备用，两源都有"代码"和"最新价"列，代码规范化去前缀。**真实验证**：run 24759960954 日志 `[!] 主源-em 全部重试失败，切换备用源 / [OK] 备用源-1 成功取到数据 / 分段筛选后: 88 只（原 581）`——新浪源兜底成功，段大小正常。市场扫描 push 8050 bytes / 5 个 ai_recommendations。**遗留**：初始批量阶段仍浪费 11.5 分钟（3 次 retry × 90s timeout）→ BUG-041 优化。 |
| 2026-04-22 | BUG-041 | BUG-040 初始批量阶段浪费 11.5 分钟在东财重试和等超时上 | data_fetcher.py batch_fetch_with_timeout | 6c11217 | ✅ 已修复（由 BUG-042 闭环：has_alt 判断避免无备用源时过早 break）。**解决方法**：timeout 30s / retry 2 / ConnectionReset 立即切源。**遗留**：没备用源的接口（get_all_stocks / get_batch_roe_data）也被立即 break 了，导致 0 次重试直接失败——比之前更糟。run 24762627641 验证：get_all_stocks ConnectionReset → 立即 break → 没 alt → 所有源失败 → 段候选 0。 |
| 2026-04-22 | BUG-042 | BUG-041 让没备用源的接口也立即 break，ConnectionReset 后无任何重试直接死 | data_fetcher.py batch_fetch_with_timeout + get_all_stocks | d2c1b35 | ✅ 已修复 + 真实环境验证通过。**根因**：BUG-041 的"ConnectionReset 立即切源"逻辑没区分有无 alt。**解决方法**：(1) batch_fetch_with_timeout 在 break 前判断 `has_alt = src is func and len(sources) > 1`；(2) get_all_stocks 加 alt_funcs 从 ak.stock_zh_a_spot 提取代码+名称列作为兜底。**真实验证**：run 24762965794 完整跑通：主源 em ConnectionReset → 立即切 sina 备用源 → get_all_stocks + get_realtime_quotes 双双兜底成功 → 段 1 60 只全扫完 / 成功 56 失败 4 / 段 1 完成：候选 6 推荐 6。conclusion=success，这是 4 天修 bug 来第一次完整走通全流程。市场扫描文件 push 10445 bytes 到 GitHub。 |
