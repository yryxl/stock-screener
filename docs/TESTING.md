...
| 2026-04-21 | BUG-036 | BUG-034 副作用：批量数据接口被 20s timeout 误杀，段大小从 60 暴涨到 728 致 75min 超时 | data_fetcher.py 4 个批量接口 | （待提交）| ✅ 已修复。**根因**：BUG-034 给 safe_fetch 默认 20s timeout 是为了保护单只调用，但批量接口（stock_yjbb_em 全 ROE / stock_zh_a_spot_em 全行情 / fund_etf_spot_em / stock_info_a_code_name）数据量大，正常需要 30-60s，被 20s 切断后 → fallback 用全市场分流 → 段大小翻 12 倍 → 75min 超时雪崩。日志证据：[超时20s] stock_yjbb_em 出现 2 次 + 段 1 候选从 60 → 728。**解决方法**：4 个批量接口显式传 `timeout=90`，单只调用保持默认 20s。 |
