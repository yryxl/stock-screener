...
| 2026-04-22 | BUG-041 | BUG-040 初始批量阶段浪费 11.5 分钟在东财重试和等超时上 | data_fetcher.py batch_fetch_with_timeout | （待提交）| ✅ 已修复（本地 2 组测试通过）。**根因**：run 24759960954 日志分析——初始阶段东财连不上，retry 3 次 × 90s timeout = 270s，外加 ROE 接口同样问题 = 11.5 分钟白等。GHA IP 被封时重试同一个源没意义，等超时更浪费。**解决方法**：(1) `timeout_sec` 默认 90s → 30s；(2) `retry` 默认 3 → 2；(3) `delay` 默认 5s → 3s；(4) **关键：识别 ConnectionReset/ConnectionError/aborted/refused 这类网络错误 → 立即 `break` 跳出 retry 循环，直接切换备用源**（不浪费时间重试同源）。本地测试：ConnectionError 主源只调 1 次即切源 / 一般异常仍 retry 2 次。预期 worst case 初始阶段从 11.5 分钟降到 4 分钟以内，给深度分析省出 7-8 分钟。 |
