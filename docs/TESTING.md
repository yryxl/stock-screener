...
| 2026-04-21 | BUG-039 | BUG-037 回归：safe_fetch 的 alarm 被移除后批量接口无超时保护，段大小再次暴涨到 728 | data_fetcher.py 批量接口 4 个函数 | 8031a6f | 🟡 部分修复（被 BUG-040 增强）。**根因**：BUG-037 为了避免 SIGALRM 嵌套冲突，把 safe_fetch 里的 alarm 全移除了。**解决方法**：新增 `batch_fetch_with_timeout(func, timeout_sec=90)` 工具。**后续发现**：夜间 cron 跑时遇到 `ConnectionResetError(104, 'Connection reset by peer')`——不是超时问题，是 GHA IP 段被东财主动断开。原版代码直接返回 None → 候选 0 只 → 数据为空覆盖。本质问题留给 BUG-040。 |
...
| 2026-04-22 | BUG-041 | BUG-040 初始批量阶段浪费 11.5 分钟在东财重试和等超时上 | data_fetcher.py batch_fetch_with_timeout | 6c11217 | 🟡 部分修复（引入 BUG-042）。**解决方法**：timeout 30s / retry 2 / ConnectionReset 立即切源。**遗留**：没备用源的接口（get_all_stocks / get_batch_roe_data）也被立即 break 了，导致 0 次重试直接失败——比之前更糟。run 24762627641 验证：get_all_stocks ConnectionReset → 立即 break → 没 alt → 所有源失败 → 段候选 0。 |
| 2026-04-22 | BUG-042 | BUG-041 让没备用源的接口也立即 break，ConnectionReset 后无任何重试直接死 | data_fetcher.py batch_fetch_with_timeout + get_all_stocks | （待提交）| ✅ 已修复（本地 2 组测试通过）。**根因**：BUG-041 的"ConnectionReset 立即切源"逻辑没区分有无 alt，对没备用源的接口（get_all_stocks / get_batch_roe_data）变成"0 重试直接放弃"。run 24762627641 只跑了 96 秒就 success 结束，market_scan_full_p1.json 写了 120 bytes 空文件（candidates=0）。**解决方法**：(1) batch_fetch_with_timeout 在 break 前判断 `has_alt = src is func and len(sources) > 1`——只有有备用源时才立即 break，没备用源仍然 retry；(2) 同时给 get_all_stocks 加 alt_funcs：从 ak.stock_zh_a_spot 提取代码+名称列作为兜底（因为 stock_zh_a_spot 是新浪源，和东财 stock_info_a_code_name 独立，更容易绕过封禁）。本地测试：无 alt 仍 retry 2 次 / 有 alt 立即切源。 |
