...
| 2026-04-21 | BUG-035 | 用户截图 Tab 资产配置："查推荐 ETF"按钮点了没反应 | app.py H2 按钮显示条件 | （待提交）| ✅ 已修复。**根因**：按钮显示条件（偏差 >3pp 绝对值）比推荐生成条件（缺位 ≥5pp）更宽，导致"超目标"（如高股息 +5.8pp）和"临界缺位"（如黄金 -5.0pp）类别显示了按钮但下方根本没对应的 expander，点击 rerun 后什么也不开。**解决方法**：(1) 提前调 `get_recommendations_from_allocation` 拿到真实推荐集合；(2) 按钮仅在 `_ac in _rec_classes` 时显示；(3) 有偏差但没推荐的类别改为 `st.caption` 友好说明（超目标→"应减仓非加仓"，偏差小→"无需补仓"）；(4) 下方 ETF 推荐区复用已算好的 `_recs_cache` 避免重复调用。 |
