...
| 2026-04-21 | BUG-038 | BUG-037 让段跑完了，但 market_scan_full_p1.json 还是没 push 到 GitHub——git add 是"全有或全无"原子行为 | daily_screen.yml git add 参数列表 | （待提交）| ✅ 已修复（本地 repro 验证）。**根因**：yml 里把 `market_scan_full_p1.json market_scan_full_p2.json ... market_scan_full_p6.json` 6 个显式文件名列出来给 git add。**但每次只跑 1 个段**（比如 full_p1），所以 p2-p6 不存在 → **git add 遇到 pathspec 不匹配会 fatal 退出 128 + 拒绝 stage 任何文件**。本地 repro 验证：`git add existing.json nonexistent.json` → staged 为空。即使有 `2>/dev/null \|\| true` 兜底，也只是让 shell 不退出，但 index 依然是空的。DEBUG 打印显示 "market_scan_full_p1.json 11649 bytes" 明明存在 → git add 后 staged 空 → commit 步骤 "Everything up-to-date"。**解决方法**：yml 的 git add 改成 bash glob + nullglob 扩展——只传入实际存在的文件给 git add。另加 `for f in ...; do [ -f "$f" ] && git add "$f"; done` 保护 tracked 文件。 |
