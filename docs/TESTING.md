...
| 2026-04-21 | BUG-039 | BUG-037 回归：safe_fetch 的 alarm 被移除后批量接口无超时保护，段大小再次暴涨到 728 | data_fetcher.py 批量接口 4 个函数 | （待提交）| ✅ 已修复。**根因**：BUG-037 为了避免 SIGALRM 嵌套冲突，把 safe_fetch 里的 alarm 全移除了，改成在 screener for 循环外层设 60s alarm。但这导致批量接口（stock_zh_a_spot_em / stock_yjbb_em 等）在 for 循环之前调用时**失去了所有超时保护**——stock_zh_a_spot_em 卡住时没人能切断它，最终 `get_realtime_quotes()` 返回空，ROE 筛选 fallback 到全市场池，段大小 60→728。日志铁证：run 24723677508 `分段筛选后: 728 只（原 4417）`。**解决方法**：新增 `batch_fetch_with_timeout(func, timeout_sec=90)` 工具——用独立 SIGALRM 保护批量接口。由于批量接口在 for 循环 **之前** 调用（时间上不重叠），所以和循环里的 60s alarm 不冲突。4 个批量函数改用这个工具：`get_all_stocks` / `get_realtime_quotes` / `get_etf_realtime_quotes` / `get_batch_roe_data`。 |
