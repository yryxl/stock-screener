...
# -------- A 股真实交易费用 --------
# 2026-04-21 按用户实际券商规则更新：佣金不高于 0.3%、最低 5 元
# 临界点：单笔 ≥ 1667 元才能让费率达到 0.3%（最低）；低于此值费率会被最低 5 元拉高
# 注：当前用户费率较高（行业普遍万 1-3），建议申请降佣到最低 1 元后回测结果会更乐观
COMMISSION_RATE = 0.003    # 佣金：双向 0.3%（用户实际券商上限）
