...
# ============================================================
# 3.5 国企/民企区分（TODO-032 / REQ-152 子项 B）（2026-04-17）
# ============================================================
# 设计依据：
#   A 股政策风险主要落在民企（教培/互联网/地产民营），国企更稳定
#   用户应该看到"持仓中民企占比 X%"提示，避免"看似分散，实际全压民企"
#
# 实现策略（按"宁可错过不犯错"原则）：
#   1. 高置信度白名单：明确知道的大型国企/央企（直接判国企）
#   2. 高置信度民企名单：明确知道的大型民企（直接判民企）
#   3. fallback 板块前缀：
#      - 002 / 003 / 300 / 301（深市中小创）→ 几乎全民企
#      - 600 / 601 / 603 / 688（沪市主板/科创）→ 国企比例高，但有民营巨头
#        → 不在白名单时返回"未知"，避免误判
#
# 接口失败容错：不依赖外部接口，纯本地名单 + 板块判定

# 高置信度国企白名单（中央/省/市国资控股 + 项目实际涉及股池）
KNOWN_STATE_OWNED = {
    # 大型央企 - 中字头能源/资源
    '601857': '中国石油', '600028': '中国石化', '600688': '上海石化',
    '601088': '中国神华', '601225': '陕西煤业', '601898': '中煤能源',
    '601600': '中国铝业', '601899': '紫金矿业', '600362': '江西铜业',
    # 大型央企 - 中字头基建/航空
    '601668': '中国建筑', '601186': '中国铁建', '601390': '中国中铁',
    '601800': '中国交建', '601618': '中国中冶', '601669': '中国电建',
    '601111': '中国国航', '600115': '中国东航', '600029': '南方航空',
    # 大型央企 - 通信/军工
    '600050': '中国联通', '601728': '中国电信', '600941': '中国移动',
    '601727': '上海电气', '600685': '中船防务', '600760': '中航沈飞',
    # 国有银行
    '601398': '工商银行', '601939': '建设银行', '601288': '农业银行',
    '601988': '中国银行', '601328': '交通银行', '601658': '邮储银行',
    '600036': '招商银行', '601818': '光大银行', '600015': '华夏银行',
    '601169': '北京银行', '601009': '南京银行',
    # 国有保险/券商
    '601318': '中国平安',  # 注：平安严格说是混合所有制，主要股东是深圳国资
    '601628': '中国人寿', '601319': '中国人保', '601336': '新华保险',
    '600030': '中信证券', '601211': '国泰君安', '601688': '华泰证券',
    '600837': '海通证券', '601066': '中信建投',
    # 国有电力/公用事业
    '600900': '长江电力', '600886': '国投电力', '600795': '国电电力',
    '601985': '中国核电', '003816': '中国广核', '600011': '华能国际',
    '600027': '华电国际', '600905': '三峡能源',
    # 国有交通运输
    '601006': '大秦铁路', '601333': '广深铁路', '600056': '中国医药',
    '601816': '京沪高铁', '601021': '春秋航空',
    # 国有酒企（贵州/四川/山西国资）
    ...
    # 国有钢铁/有色
    '600019': '宝钢股份', '000898': '鞍钢股份', '600808': '马钢股份',
    # 国有医药
    '600276': '恒瑞医药',  # 实际是民营，移到 KNOWN_PRIVATE
    '600085': '同仁堂', '600436': '片仔癀',  # 漳州市国资
    # 国有食品/农业
    '600887': '伊利股份',  # 国资 + 战投，主要是地方国资
    '000895': '双汇发展',
    # 深市主板里的国企（白名单优先于板块前缀粗判）
    '000538': '云南白药',  # 云南省国资委控股
    '000661': '长春高新',  # 长春国资委控股
    '000063': '中兴通讯',  # 深圳市国资委控股
}
# 修正：把误归的恒瑞医药从国企移到民企（恒瑞是江苏连云港的民营药企）
KNOWN_STATE_OWNED.pop('600276', None)

# 高置信度民企白名单（沪市主板里的知名民营巨头，避免被前缀判错）
KNOWN_PRIVATE = {
    '600276': '恒瑞医药', '600196': '复星医药', '600887_NOTE': '伊利国资别错',
    '603259': '药明康德', '603501': '韦尔股份', '603799': '华友钴业',
    '600585': '海螺水泥',  # 实际海螺是国资，先注释
    '600690': '海尔智家',  # 海尔是混合所有制，主要为青岛国资
}
# 修正
KNOWN_PRIVATE.pop('600585', None)
KNOWN_PRIVATE.pop('600690', None)
KNOWN_PRIVATE.pop('600887_NOTE', None)


def is_state_owned(code):
    """
    REQ-152 子项 B：判断股票是否为国企

    返回：
      (status, reason)
        status: True (国企) / False (民企) / None (未知/不确定)
        reason: 判定依据文案

    设计原则：
      - 宁可错过不犯错：不确定的归"未知"，不强行判定
      - 不依赖外部接口（避免网络/数据失败影响主流程）
    """
    if not code:
        return None, "代码为空"
    ...
    # 1. 高置信度白名单
    if code in KNOWN_STATE_OWNED:
        return True, f"已知国企（{KNOWN_STATE_OWNED[code]}）"
    if code in KNOWN_PRIVATE:
        return False, f"已知民企（{KNOWN_PRIVATE[code]}）"

    # 2. ETF 类（5 开头）跳过
    if code.startswith('5') or code.startswith('1'):
        return None, "ETF/基金代码，不适用国企/民企判定"

    # 3. 板块前缀粗判
    if code.startswith(('002', '003', '300', '301')):
        # 深市中小板/创业板：几乎全民企
        return False, "深市中小创板块（几乎全民企）"
    if code.startswith(('600', '601', '603', '605', '688')):
        # 沪市主板/科创板：国企比例高但有民营巨头
        return None, "沪市主板/科创板（白名单未匹配，建议手动确认）"
    if code.startswith(('000', '001')):
        # 深市主板：混合（深发展、万科、平安等有国企背景，多数为民企）
        return None, "深市主板（白名单未匹配，建议手动确认）"
    if code.startswith('900') or code.startswith('200'):
        return None, "B 股，不区分国企/民企"

    return None, f"未知前缀 {code[:3]}"


def calc_holdings_ownership_breakdown(holdings):
    """
    REQ-152 子项 B：计算持仓的国企/民企占比

    输入：holdings（list of dict，需含 code 字段）
    返回：dict {
      'state_owned_count': X, 'state_owned_pct': X.X,
      'private_count': X, 'private_pct': X.X,
      'unknown_count': X, 'unknown_pct': X.X,
      'private_alert': bool,  # 民企占比 > 70% 时为 True
      'detail': [{'code', 'name', 'status_label', 'reason'}, ...]
    }
    """
    if not holdings:
        return None
    ...
    total = state_owned + private + unknown
    if total == 0:
        return None

    private_pct = private / total * 100
    return {
        'state_owned_count': state_owned,
        'state_owned_pct': round(state_owned / total * 100, 1),
        'private_count': private,
        'private_pct': round(private_pct, 1),
        'unknown_count': unknown,
        'unknown_pct': round(unknown / total * 100, 1),
        'private_alert': private_pct > 70,  # 民企 >70% 触发警示
        'total': total,
        'detail': detail,
    }
