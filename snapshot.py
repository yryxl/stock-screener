...
def save_snapshot():
    ...
    market_cache = load_json("market_scan_cache.json")
    ...
    snapshot = {
        ...
        # 关注表状态（每只股票的信号+PE+价格）
        "watchlist_signals": daily.get("watchlist_signals", []) if isinstance(daily, dict) else [],

        # 持仓状态
        "holdings": holdings if isinstance(holdings, list) else [],
        "holding_signals": daily.get("holding_signals", []) if isinstance(daily, dict) else [],
